...
CREATE TABLE IF NOT EXISTS `popularity_ledger` (
  ...
  `decay_coefficient` INT NOT NULL DEFAULT 100 COMMENT '衰减系数 (例如100代表1.00，50代表0.50)',
  `final_popularity_change` BIGINT NOT NULL COMMENT '最终人气值变动（原始值 * 衰减系数）',
  `current_total_popularity` BIGINT NOT NULL COMMENT '此条记录发生后，目标对象的总人气值',
...
CREATE TABLE IF NOT EXISTS `popularity_summary` (
  ...
  `target_type` VARCHAR(50) NOT NULL COMMENT '目标类型 (player, team, spy, pool)',
  `target_id` INT NULL COMMENT '目标ID (player_id, team_id)，pool类型可为空',
  `current_popularity` BIGINT NOT NULL DEFAULT 0 COMMENT '当前总人气值（所有 final_popularity_change 的累加）',
