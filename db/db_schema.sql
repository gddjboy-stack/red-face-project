...
ALTER TABLE suspicion_votes ADD CONSTRAINT fk_suspicion_votes_user_id FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE ON UPDATE CASCADE; -- Assuming a users table exists or will be created
...
-- Add Foreign Key for collect_state if target_id refers to player_id, team_id, or round_id
-- This requires conditional foreign keys or application-level enforcement due to dynamic target_type
-- For now, we will rely on application-level integrity for collect_state.target_id

-- Note: A 'users' table is implicitly referenced by suspicion_votes.user_id and tokens.user_id.
-- It is assumed that a 'users' table will be created to manage user information.
```
